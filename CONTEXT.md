...
The immutable `adoc.managed_field_provenance.v0` manifest associates an exact Workspace Canonical Identity, Managed Version ID and content digest with materially contributing native Source Assertion row UUIDs for each field selector. Selectors are `/body` or `/fields/<one canonical JSON-pointer token>`; native storage checks the exact authored string field. The strictest contributing historical snapshot classification and authored object/field floors determine visibility. Absent authored floors mean public; missing contributing classification means unresolved, never public. A policy administrator explicitly approves each historical snapshot classification; approval creates no verification fact and rewrites no existing source or knowledge bytes. E6.2.T2 consumes current classifications to return authorized scalar siblings without rewriting canonical bytes; incompatible whole-object paths remain denied. Returned display labels inherit copied metadata sensitivity, while direct-source access markers preserve source identity/classification for native audit.
_Avoid_: external envelope IDs in native assertion links, silently public unknown provenance, mutable historical classification
...
### Governed field declassification (E6.2.T4)

The immutable `adoc.managed_field_declassification.v0` detail binds an existing bare state-event ordinal/digest to exact version/content/manifest, selected scalar fields, prior/new classes, native assertion UUIDs, authenticated principal/session/decision/policy, rationale/date and true-only restricted-evidence withholding. Native `knowledge.declassify` admission serializes a strictly descending per-selector chain; only current effective details apply, while current source ACLs and missing provenance still fail closed. A compact private projection reference never authenticates itself. For an otherwise unreadable object, only approved scalar values and identity/hash survive; source is `{}`, with all unselected/dedicated/lifecycle/reference/evidence metadata withheld. Existing bare event and canonical content bytes remain unchanged. Model or human-looking patch proposer labels cannot authorize lowering; proposal reasons may suggest escalation. See [contract guide](docs/agent/v0/managed-field-declassification.md).
