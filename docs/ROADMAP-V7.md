...
- Extend `expanded_pilot.rs` graph and retrieval assertions for the four new kinds; extend the V6.4 TB5 loop test with one apply against a new-kind object (e.g. marking the task `done` via `update_fields`). The loop test runs against a tempdir copy and pins rewritten files byte-for-byte with goldens under `crates/adoc-cli/tests/fixtures/` — the task apply gets its own `.after.adoc` golden beside the V6.4 one. The diagnostic budget is body-edit-invariant, but the task apply flips `task.overdue` off (the task is no longer `open`) — the post-check assertion must account for the 6→5 transition inside the loop test while the pristine in-repo tree keeps the 6-warning budget.
...
- The guard: extend `crates/adoc-mcp/tests/manifest_guard.rs` to parse the tool list out of `README.md` and `docs/mcp-agent-gateway.md` and assert exact set-equality with the registered manifest — names, not counts, so the failure message says *which* tool is missing, not just that a number is off. One more assertion in the same test derives the README kind list from `BlockKind::ALL`, so V6.5 landing turns the README stale loudly instead of silently. This is the docs-truth rule made executable: the next engineer who adds a tool or a kind gets a red test naming the files to touch.
...
Commit shape: `docs(v7): ADR-0041 docs-truth guards` → `docs: true up README/ROADMAP/gateway to shipped surface (V7.1)` → `test(mcp): manifest guard asserts published tool and kind lists match registry (V7.1)`.

Acceptance: `cargo test -p adoc-mcp --test manifest_guard --locked` exits 0; deliberately removing one tool name from `README.md` makes it fail naming that tool. `ROADMAP.md` "Current Status" names V6.4 as shipped and this file as next. The enumerated debt above is gone: README lists 14 tools and 11 kinds, the gateway doc covers the registered set, and a grep over `README.md`, `docs/ROADMAP.md`, and `docs/mcp-agent-gateway.md` finds none of the enumerated stale phrases (this file's own debt list is quotation, not debt).
...
- **ADR-0041** — Docs-truth guards: published tool and kind lists are asserted against the code registry (`#[tool]` manifest, `BlockKind::ALL`) by `manifest_guard.rs`-style tests; set-equality on names, not counts; the guard is the mechanism, hand-maintenance is retired.
