...
The envelope reports project root, config discovery, resolved local paths, refresh diagnostics, graph/search artifact status, artifact diagnostics, and readiness booleans for retrieval, semantic search, and patch validation.

Use this schema before retrieval, semantic search, or patch validation so agents can decide whether a check or build refresh is required.
