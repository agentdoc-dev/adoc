...
`agent_instruction` nodes are emitted into the graph artifact (`adoc.graph.v6`) with `kind: "agent_instruction"`, the typed `trust` (top-level node field; these nodes carry no `status`, per ADR-0039), the `scope`, and both action sets, and fold into the retrieval surface (`adoc.retrieval.v1`) like any other Knowledge Object. No new envelope version is introduced.
