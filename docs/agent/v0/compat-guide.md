...
V4 introduces **Compatibility Mode**: AgentDoc accepts `.md` source alongside `.adoc`, parses it with a CommonMark + GFM parser, and emits the same `adoc.graph.v6` artifact and `dist/docs.html` as native AgentDoc Source. Compatibility Mode is selected purely by file extension; `.adoc` files always validate under Strict Mode, `.md` files always validate under Compatibility Mode. There is no flag or config block to toggle this (ADR-0022).
...
Since V1.7.1 (ADR-0040), `.md` prose **is searchable**: `adoc_search` returns `record_type: "prose"` records blended with Knowledge Objects. Use prose hits as orientation context; they never satisfy the citation shape. A search over a `.md`-only project that matches nothing still returns the `retrieval.no_knowledge_objects_consider_migration` warning as the structural explanation, not a bug.
...
| `retrieval.no_knowledge_objects_consider_migration` | A search returned zero records against a graph that has prose blocks but no Knowledge Objects. Surface this verbatim; do not suppress it. |
