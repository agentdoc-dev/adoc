...
Source objects appear in `adoc.graph.v6` nodes with `kind: "source"`. Evidence kind and path or URL are projected into the node's `fields` map.
...
`source` nodes are emitted into the graph artifact (`adoc.graph.v6`) with:
