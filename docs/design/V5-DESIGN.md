...
- Source object resolution at retrieval time: whether `evidence_ref:` produces an extra reference edge in the graph artifact or a per-record projection only. **Resolved in V5.8 (ADR-0032): both** — a `{kind, reference}` entry in the node's typed `evidence` array AND a derived `GraphEdgeKind::Evidence` edge (not a user `GraphRelationKind`). `Evidence` was also refactored to the literal `Inline { kind, value } | ObjectRef` shape (collapsing `reviewed_by`/`human_review` to `human_review`), and `decision` gained full symmetric evidence.
...
## Implementation Deviations (accepted, post-audit 2026-06)

A three-way audit (PRD ↔ this contract ↔ code) after V5.10 shipped found the implementation substantially conformant. Two contract deviations were fixed in code (the missing `schema.contradiction_claim_not_a_claim` diagnostic; the missing unresolved-contradiction owner re-assert obligation trigger). The graph-node `status` slot overload (warning/constraint carry **Severity**, agent_instruction carries **Trust** in the `status` slot) is remediated by dual-emitting dedicated `severity`/`trust` fields — see ADR-0035 for the decision and the deferred v4 cleanup. The following as-built deviations are accepted without code changes:

- **Pilot layout.** `examples/expanded-pilot/` groups objects in per-domain files (`auth/claims.adoc`, `security/policies.adoc`, …) under `auth/`, `billing/`, `security/`, and `meta/`, with `agentdoc.config.yaml` at the root — not the per-object files under `src/` with a `README.adoc` sketched in the Workspace Layout section. The grouped layout matches how real docs trees are authored and is what the V5.9 acceptance tests pin.
- **Validation rule location.** Per-kind required-field invariants live in the aggregate constructors (`domain/knowledge_object/<kind>.rs`); cross-aggregate workspace rules (`contradiction_claims_resolve.rs`, `evidence_ref_resolves.rs`, …) live flat in `infrastructure/validate/` alongside the existing cross-cutting rules. The `infrastructure/validate/objects/` directory sketched in the Workspace Layout section was never needed — SRP holds at the rule-per-file level without it.
- **`BlockKind` visibility.** `BlockKind` is `pub(crate)`, not public, so the planned `#[non_exhaustive]` is moot — downstream consumers see kinds only as strings via `KnowledgeObjectRecord`. This matches the "no new function exports; new modules are `pub(crate)`" rule in the Public Core API Additions section, which supersedes the per-enum `#[non_exhaustive]` listing for crate-private enums.
- **`FieldChange::Severity`/`FieldChange::Trust` payloads.** These carry `Option<String>` rather than typed values because they are projections of the untyped graph-node status slot. See ADR-0035; typed payloads arrive with the v4 cleanup.
