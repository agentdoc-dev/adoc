...
Every decision records the required scoped-grants input status `membership_evidence`. A group-bearing grant forbids `not_applicable`: it records `current` when every relevant membership input was established, or `insufficient_context` when another potentially grant-conferring input was unavailable or not yet established. Every `insufficient_context` status retains `membership_unavailability_evidence` for every unresolved relevant input, including when an earlier gate wins. A manual entry identifies the exact stable group id and retained lifecycle-failure record; when `evaluation_time` is valid it also requires the name in effect at that instant, while `evaluation_time_invalid` omits `group_name` because no valid instant exists against which to resolve it. An external entry additionally identifies the source kind, identity link, unavailable state, retained expiry, failure, or pending-state record, and the binding-mode epoch actually in effect. For `epoch_observation_pending`, that epoch may be the prior `suggestion_only` or `disabled` mode while `state_record_id` identifies the pending request for a grant-conferring transition; no new grant epoch is invented before its sweep completes. Replay verifies group-name history and temporal state only when the instant is valid; `evaluation_time_invalid` verifies stable input identity and state-record ownership without a temporal claim against the malformed instant. Completeness is evaluator-owned and fails closed. The registered states are exact: `lifecycle_unavailable` for manual lifecycle failure; `observation_expired` for connector or OIDC evidence expiry; `connector_read_failed`, `link_read_pending`, `link_read_failed`, and `epoch_observation_pending` only for connector-read sources; and `oidc_authentication_pending` only for claim-only `oidc_group`. Any OIDC unavailable input is human-only. An OIDC `observation_expired` entry carries its own historical `identity_session_id`, which must equal the session retained by the expired observation named by `state_record_id`; `oidc_authentication_pending` carries no session on that entry because no observation exists yet, although the envelope may independently carry the evaluation session required by a sibling resolved OIDC observation. When the scoped-grants stage owns the outcome, `insufficient_context` records reason `membership_evidence_unavailable`; earlier identity, evaluation-time, hard-deny, or source-ACL outcomes and a decisive `explicit_deny` retain precedence. `no_grant` with `current` retains `membership_absence_evidence` for every relevant confirmed absence: a manual entry identifies the group whose complete membership lifecycle history is replayed for the envelope principal at `evaluation_time`, while an external entry retains the exact group, binding mode epoch, identity link, negative observation, and source event or run. A group-bearing grant is a necessary but not sufficient condition for an empty array; the array is empty only when no relevant membership was established absent. `no_grant` with `not_applicable` carries no absence evidence.

Every external binding retains its complete effective membership-freshness policy history: each version records the freshness window, refresh mechanism and schedule, and effective instant. This membership-freshness policy is distinct from the connector source-ACL policy owned by E2.6. Every observation retains `fresh_until`, derived from `observed_at` under the versioned freshness policy retained by the exact binding; replay resolves the historical version in effect, recomputes the deadline, and requires `fresh_until` to equal that recomputation. The exception is a positive observation from a removal-event read: when a prior positive observation exists for the same group, binding, and identity link, `fresh_until` is capped at the deadline of the latest prior positive observation and equals the earlier of that cap and the historical recomputation. When no prior positive observation exists, no cap applies and the ordinary historical recomputation governs. At authorization time replay also resolves the policy version in effect at `evaluation_time`; the observation may confer only through the shorter of retained `fresh_until` and the deadline recomputed from `observed_at` with that evaluation-time freshness window. A shorter policy caps existing observations immediately, while a longer policy never revives or extends them. An uncapped observation's effective deadline must follow `effective_at`, so it is never expired before becoming eligible. A capped observation whose `fresh_until` does not follow `effective_at` is born inert: it is retained for replay, confers nothing, and the input records `observation_expired`. A synchronization run whose produced observations cannot satisfy the uncapped ordering is rejected as a freshness-policy misconfiguration rather than emitting silently inert observations. A period of connector unavailability cannot extend the deadline, so a stale positive observation is inert until a fresh current-state read succeeds. When the scoped-grants stage cannot establish a membership input—including an unresolved manual-membership lifecycle record, an expired external observation, an unavailable connector read, a pending or failed suspended link read, a grant-conferring connector reconfiguration awaiting its transition sweep under the prior epoch, or a newly opened `oidc_group` epoch awaiting authentication—the decision records `membership_evidence: insufficient_context` and reason `membership_evidence_unavailable` with a null basis; a consequential decision yields `insufficient_context`, while a nonconsequential decision yields `deny`. `no_grant` remains reserved for a confirmed absence of an applicable grant. For claim-only `oidc_group`, the effective deadline is additionally capped at the cited identity session's expiry.
...
A connector membership observation resolves against exactly one retained source event or synchronization run: its `observed_at` equals the current-state read instant retained with that event or falls within that run, and its `effective_at` equals that event's ingestion-commit instant or that run's completion instant. Replay also verifies that the source record belongs to the observation's retained binding and that its event or retained per-principal run membership subject resolves through the observation's retained external identity link (`external_identity_link_id`); any mismatch fails closed. The event ingestion transaction commits only after the successful read it prompted, so `observed_at` cannot follow `effective_at`. Event polarity only triggers the current-state read; the read result determines observation polarity. A positive event whose read confirms absence records a negative observation and no positive observation. A removal event whose read still shows membership records a positive observation and no negative observation. Its `fresh_until` is capped at the deadline of the latest prior positive observation for the same group, binding, and identity link when one exists; when no prior positive observation exists, the ordinary historical-policy deadline applies. The signal therefore never renews prior positive membership. A cap at or before `effective_at` makes the new observation born inert and the input records `observation_expired`. The event-versus-read contradiction is retained and operator-visible, and the binding's next scheduled resynchronization bounds revocation propagation.
...
The `oidc_group` source kind is claim-only in V1 and valid only for a human principal authenticated through an E2.3 workspace identity session. A token that requires an out-of-band group lookup produces no `oidc_group` observation. Its `source_event_id` identifies the exact retained authenticated identity-session event carrying its token issuance, validation/ingestion-commit, and session-expiry instants. Validation of a freshly issued and verified ID token at authentication is the current-state read, the token issuance instant is `observed_at`, and the validation/ingestion-commit instant is `effective_at`. Every positive or negative `oidc_group` observation requires the authorization principal's `identity_session_id`, which must equal the identity session retained by source_event_id; the observation is valid only during that matching session, no later than the cited identity session's expiry. A later authentication without the claim records a session-bound negative observation and cannot reuse an earlier session's positive observation. Provider group-claim revocation therefore takes effect for each session at its next authentication and is bounded by the configured session lifetime under E2.3.T5; no out-of-band membership sweep exists. A grant-conferring reconfiguration opens the new epoch only after provider-configuration validation, carries forward no prior observations, and each principal regains membership only when a later authentication records a new positive observation in that epoch.
