...
Every external membership observation retains `fresh_until`, derived from `observed_at` under the versioned freshness policy retained by the exact binding. Replay recomputes the deadline and permits the observation to confer only when `evaluation_time` does not exceed `fresh_until`; connector unavailability cannot extend it, so a stale positive observation is inert until a fresh current-state read succeeds. For claim-only `oidc_group`, `fresh_until` is additionally capped at the cited identity session's expiry.
