...
Each retained membership observation identifies one exact external binding. Replay verifies that the observation's retained binding equals the enclosing `binding_id` and that the retained binding's AgentDoc group equals the enclosing group id; any mismatch fails closed.
