...
| `adoc.semantic_context.v0` | shipped | adoc 0.4.0 | adoc 0.4.0 (domain validator and MCP schema resource; receipt integration E3.1.T5) | exact-match reader; deterministic digest-bound exact revisions and closed citation handles; JSON Schema is preflight only and `adoc-core` owns semantic validation |
