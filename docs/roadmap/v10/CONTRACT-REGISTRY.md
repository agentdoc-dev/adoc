...
| `agentdoc.cloud.external_promotion_attestation.v0` | cloud | E4.3 | closed external-promotion attestation binding one candidate and policy to the policy-declared issuer and attestation type plus trusted issuer evidence established independently by a provider-authenticated connector/service or cryptographic verification; its immutable Source Assertion proves exact-byte provenance only |
