...
| `agentdoc.evidence_contract.v0` | shipped | adoc 0.4.x | Cloud G1A evaluator (E4.7); later G1B/layer-specific evidence cohorts (E7.6/E9.1) | exact-match schema for versioned YAML evidence contracts; every contract freezes its cohort, population and duration floors, named numerator/denominator rules, exclusions, thresholds, stop-ship conditions, and approvers before eligible observations; below-floor rates are descriptive `insufficient_evidence` and promote nothing; a material post-eligibility change closes the cohort version and starts a new one |
<!-- /registry:envelopes-shipped-adoc -->
