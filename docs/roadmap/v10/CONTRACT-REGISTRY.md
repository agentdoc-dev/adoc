...
| `adoc.migration_request.v1` | unreleased | adoc 0.4.x (U3.3 P3a) | U3.3 P3b Cloud E7 worker; MCP schema resource | exact-match successor adding `inspection_id`, `inspection_digest` and `starting_point` (`recorded_history`\|`fresh`); mode participates in request digest; v0 stays recorded-history only and refuses v1 fields |
| `adoc.migration_receipt.v1` | unreleased | adoc 0.4.x (U3.3 P3a) | U3.3 P3b Cloud E7 worker; MCP schema resource | v0 receipt plus `config_profile` (`committed`\|`generated_default_v1`) and `config_digest`; no-config sources are fresh-only |
| `adoc.migration_import.v1` | unreleased | adoc 0.4.x (U3.3 P3a) | U3.3 P3b Cloud native candidate importer; MCP schema resource | v0 bundle shape embedding a v1 request |
| `adoc.migration_qualification.v1` | unreleased | adoc 0.4.x (U3.3 P3a) | U3.3 P3b Cloud qualification admission; MCP schema resource | v0 outcome embedding a v1 request; policy `1` for recorded_history, `fresh.1` for fresh; any other pairing refuses |
| `adoc.migration_qualification_receipt.v1` | unreleased | adoc 0.4.x (U3.3 P3a) | U3.3 P3b Cloud qualification admission; MCP schema resource | adds `starting_point`; `fresh` requires policy `fresh.1`, every object `eligible: false` with sole reason `fresh_review_required` (zero initialization targets, never an eligibility assertion); source/graph hashes unchanged |
...
| `adoc.repository_inspection_request.v0` | unreleased | U3.3 P2b Cloud inspection runtime port | adoc 0.4.x `adoc repository inspect` CLI; MCP schema resource | exact-match read-only inspection request; binds workspace, provider repository ID, ref, full nonzero lowercase Git commit SHA-1, explicit evaluation date and exact runtime version/binary digest pin; unknown fields and runtime mismatch refuse |
| `adoc.repository_inspection_receipt.v0` | unreleased | adoc 0.4.x `adoc repository inspect` CLI | U3.3 P2b Cloud inspection runtime port; MCP schema resource | exact-match bounded read-only receipt; request digest, runtime pin, config state `valid`/`invalid`/`absent`, committed or runtime-owned `generated_default_v1` profile with generated config bytes/digest, eligible-file count and parsed-item count (pages plus Knowledge Objects from the validator's own parse; null when any source is unparseable), config digest, sorted path/content-hash manifest digest sorted `manifest` list of `{path, sha256}` (null when not enumerated; its compact JSON hashes to `manifest_digest`) and at most 64 diagnostic codes; unsafe sources and native import source-count/byte limits refuse without a receipt; grants no import, registration or promotion authority |
...
| `agentdoc.cloud.migration_request.v1` | cloud | U3.3 P3 | closed prepare transport: repository/connector/source container IDs, ref, exact nonzero Git OID, `previous_preview_id` (null = no predecessor, never fresh), inspection ID/digest and `starting_point`; v0 remains supported unchanged |
...
| `gate.policy_missing` | planned (E8.1.T4) | Cloud `evaluate_github_attestation_gate_decision_v1` raises this fixed refusal (SQLSTATE `42501`) when the repository has no `repository_gate_policies` row; this departs deliberately from the shipped `adoc.gate_result.v0` absent-mode `advisory` default for attested evaluation only, because an attested approval needs an explicit gate policy to evaluate against, while native evaluation never reaches this evaluator and still derives `advisory`, so no repository silently gains a blocking gate; checked first, before attestation facts or `governance.attestation_not_current`; returned to the attested evaluator caller before any gate decision is written, so it is a blocking refusal, not an `adoc.gate_result.v0` reason, and never replaces shipped `gate.approval_missing`; admission fails closed rather than falling back to native or an older policy. Separately, the E8.1.T4 approval-source mode mutation raises the same code when asked to select `github_attestation` for a repository with no `repository_gate_policies` row; that governed write is refused, so no mode revision, receipt or audit event is retained. Gate-policy revisions are append-only (Cloud `20260905000000_create_gate_evaluator.sql` refuses update, delete and truncate), so valid configuration never reaches the evaluator refusal, which remains a fail-closed backstop |
