...
| `adoc.git_proposal_references.v99` | fixture | rejected-version fixture proving the Git proposal reference parsers (Action and Cloud) refuse unknown block versions (E8.2.T1) |
| `agentdoc.cloud.proposal_delivery.v99` | fixture | rejected-version fixture proving `/api/v1` rejects unknown proposal-delivery report versions with registered remediation (E8.2.T2) |
<!-- /registry:test-fixture-ids -->
...
The 12 `agentdoc.cloud.*` rows below whose owning slice is E8.1 inventory implementations merged to Cloud main at `e8e9c276` (cloud#169, 2026-09-25) and are `unreleased` in the sense of the status legend above; they remain in this block until Cloud's first versioned release, without claiming production deployment or activation (`docs/plans/E8.1-QUALIFICATION-PLAN.md` D-7). The E8.1 code rows in the Action, Cloud and attestation code tables carry `unreleased` explicitly, except `connect.installation_mode_conflict`, registered `planned (E8.1.T4, activation slice A0)` — the T4 tag because the activation slice A0 of that plan has no roadmap counterpart — and flipped when A0 lands. The canonical spelling for a merged-but-unreleased row is the `unreleased (<owning slice>)` status of the legend; the E5 rows' `planned (…, implemented in Cloud v0.1.0 pre-release)` annotations above are a legacy spelling of the same state and are not migrated.
...
| `agentdoc.cloud.github_approval_attestation_attempt.v0` | cloud | E8.1.T3 | canonical rejected-attempt record binding the exact Workspace, proposal exposure, reviewer identity link (for an unlinked GitHub `Bot` rejection, its immutable GitHub actor ID instead), closed status/code pair, request digest and immutable attempt ID; its digest covers the rejection outcome so display cannot substitute an uncovered status or code. An unlinked human GitHub reviewer produces no canonical attempt and no status projection; admission returns `attestation.requirements_unmet` |
| `agentdoc.cloud.github_approval_attestation_status.v0` | cloud | E8.1.T3 | display-only projection binding one retained attestation or attempt to the exact current Workspace/repository/pull request/head SHA, Proposal Set version/digest, and the ID/digest of the gate decision published for that head; Cloud attaches it only to a ready check publication, so a head with no published decision (an attested `gate.policy_missing` or `governance.attestation_not_current` refusal) or no retained attestation or attempt (an unlinked human reviewer) gets no projection; a published decision that carries a native approval or a break-glass emergency receipt also projects null; when the selected retained record is a `satisfied` attestation, the projection is null if that attestation has been invalidated or the published decision is not bound to it (so the stale-window `gate.approval_missing` fallback shows no satisfied block), while a selected attempt still projects; in each null case the Action omits the section, which grants nothing. E8.1 adds no admission-triggered publication: a retained attestation or attempt reaches the check on the head's next assessment- or approval-triggered publication (or a failure retry of either), and live webhook admission with republication ships with attested-mode activation; only the four closed attestation statuses are accepted, and the projection grants no approval authority |
| `agentdoc.cloud.github_approval_source_policy_receipt.v0` | cloud | E8.1.T4 | immutable receipt for an authorized repository approval-source mode revision; binds the prior revision, request digest, actor, audit event and authorization decision; carries no bot entries, which live in the Workspace allowlist |
| `agentdoc.cloud.github_bot_approval_allowlist_receipt.v0` | cloud | E8.1.T4 | immutable receipt for an authorized Workspace-scoped exact bot-identity allowlist revision (PRD §15.2, GOV-002); binds the Workspace, prior revision, sorted unique exact `(provider, issuer, GitHub actor ID, principal ID)` entries, request digest, actor, audit event and a Workspace-scope `policy.manage` authorization decision; an entry lifts only the non-human rejection, and only in repositories whose current approval-source mode is `github_attestation` |
...
| `agentdoc.cloud.github_approval_gate_fact.v0` | cloud | E8.1.T5 | internal gate fact binding a current typed GitHub attestation reference, proposal, policy and observation state; an invalidated or stale attestation cannot supply approval authority. Distinct from native `agentdoc.cloud.approval_gate_fact.v0`: it carries the `approval_source` discriminator of kind `github_attestation` |
| `agentdoc.cloud.gate_evidence_set.v1` | cloud | E8.1.T5 | attested gate-evidence digest input with an explicit GitHub attestation source; native v0 evidence bytes remain unchanged |
...
| `agentdoc.cloud.approved_proposal_promotion.v1` | cloud | E8.1.T5 | causal receipt for an attested approved-proposal promotion with an explicit GitHub attestation ID and digest; native promotion v0 remains decodable |
...
| `agentdoc.cloud.activation_gate_evidence.v1` | cloud | E8.1.T5 | activation digest input for exact active promotion receipts under the typed GitHub attestation reference |
...
| `agentdoc.cloud.terminal_gate_evidence.v1` | cloud | E8.1.T5 | terminal digest combining the attested gate-evidence and activation digests without changing native v0 bytes |
...
| `agentdoc.cloud.internal_integrated_trace.v1` | cloud | E8.1.T5 | internal trace whose approval and activation hops retain the exact tagged GitHub attestation ID and digest; native v0 trace readback remains unchanged |
...
| `agentdoc.cloud.export_native_fact.v0` | cloud | E6.6.T5, E7.2.T4 | Closed per-kind native_json facts for inventory members without retained wire bytes; includes exact reconciliation, rollback, and fence-release native row arms; no historical byte-attestation claim; exact version and closed schema |
...
| `agentdoc.cloud.migration_reconciliation_begin_request.v0` | cloud | E7.2.T4 | closed reconciliation successor reservation command; native derives retained child bindings and current authority; schema and MCP published |
| `agentdoc.cloud.migration_reconciliation_begin_receipt.v0` | cloud | E7.2.T4 | closed immutable reconciliation reservation receipt; compact and no embedded basis bytes; schema and MCP published |
| `agentdoc.cloud.migration_reconciliation_context.v0` | cloud | E7.2.T4 | closed transient authenticated parent frontier inspection; nullable active bindings remain coupled; schema and MCP published |
| `agentdoc.cloud.migration_reconciliation_import_receipt.v0` | cloud | E7.2.T4 | closed immutable import receipt referencing native delta bytes only by digest; schema and MCP published |
| `agentdoc.cloud.migration_reconciliation_accept_request.v0` | cloud | E7.2.T4 | closed current human reconciliation acceptance command; schema and MCP published |
| `agentdoc.cloud.migration_reconciliation_attestation.v0` | cloud | E7.2.T4 | closed immutable attributed reconciliation attestation; schema and MCP published |
| `agentdoc.cloud.migration_reconciliation_accept_receipt.v0` | cloud | E7.2.T4 | closed immutable reconciliation acceptance receipt; schema and MCP published |
| `agentdoc.cloud.migration_rollback_context.v0` | cloud | E7.2.T4 | closed authenticated rollback inspection context; exact original and selected-observation bindings; schema and MCP published |
| `agentdoc.cloud.migration_rollback_request.v0` | cloud | E7.2.T4 | closed rollback command with immutable cutover or accepted-reconciliation basis; schema and MCP published |
| `agentdoc.cloud.migration_rollback_receipt.v0` | cloud | E7.2.T4 | closed immutable compensating rollback receipt and selected source proof; schema and MCP published |
| `agentdoc.cloud.migration_fence_release_claim.v0` | cloud | E7.2.T4 | closed native rollback-associated provider fence release claim; schema and MCP published |
| `agentdoc.cloud.migration_fence_release_evidence.v0` | cloud | E7.2.T4 | closed qualified-controller terminal release evidence; schema and MCP published |
| `agentdoc.cloud.migration_fence_release_receipt.v0` | cloud | E7.2.T4 | closed immutable verified terminal fence release receipt; schema and MCP published |
| `agentdoc.cloud.migration_fence_release_context.v0` | cloud | E7.2.T4 | closed authenticated release read with nullable compact claim/release envelopes; schema and MCP published |
...
| `agentdoc.cloud.migration_http_operation.v0` | cloud | E7.1.T1 | queued prepare-operation response bound to an authenticated migration operation ID; HTTP admission does not itself complete qualification or activation |
| `agentdoc.cloud.migration_http_operation_result.v0` | cloud | E7.1.T1 | durable worker prepare result with delivery status, blocked reason and optional retained preview; no successful result is inferred from queue admission |
| `agentdoc.cloud.migration_http_cutover_operation.v0` | cloud | E7.2.T3 | queued cutover-operation response bound to the exact migration and operation IDs |
...
| `adoc.git_proposal_references.v0` | action | E8.2.T1 | exact-match Action-owned reference block for a Cloud-connected delivery, delimited by `<!-- AgentDoc-Proposal-References:v0 -->` markers around one compact JSON object carrying exactly the five references in fixed key order — `source_pr` (numeric base-repository id and PR number), `source_head_sha`, `assessment_receipt_digest` (the finalized `adoc.pr_assessment_receipt.v4` bytes, not the assessment digest), non-empty sorted unique `affected_objects` with post-patch semantic `content_hash`, and `proposal_set_digest`; ≤ 64 KiB; unknown/duplicate keys, a second marker pair or a missing field fail parsing; a run with no proposal record emits no block. Cloud validates it and never re-renders it |
| `agentdoc.cloud.proposal_delivery.v0` | cloud | E8.2.T2 | exact-match `/api/v1` delivery report (operation `proposal_delivery`) binding a proposal version and set digest, assessment receipt digest, assessed head, project prefix, mode `commit` or `pr`, delivered commit and knowledge PR; Cloud validates it against stored evidence and its own provider observation and retains every admitted attempt append-only beside the immutable Proposal Record; a delivery is not writeback and confers no approval, governance, verification, promotion or effectivity |
| `agentdoc.cloud.proposal_delivery_status.v0` | cloud | E8.2.T5 | session read projection resolving (Workspace, proposal-set digest) to the exact proposal, its accepted deliveries with commit/PR links, refused attempts and a derived status: `missing` (no attempt admitted), `pending` (attempts admitted, none accepted, including refused-only), `delivered` (an accepted delivery whose version and assessment are current) or `stale` (accepted deliveries only for a superseded version or assessment); `status` never claims approval |
<!-- /registry:envelopes-planned -->
...
| `action.attestation_bot_rejected` | unreleased (E8.1.T3) | Action check wrapper for the canonical Cloud code `attestation.bot_approver_rejected` — one documented mapping, no competing suffix |
...
| `governance.promotion_not_authorized` | planned (E5.5.T1, implemented in Cloud v0.1.0 pre-release) | `promote_approved_proposal_candidate` refuses missing or mismatched proposal lineage/candidate authority, non-current approval or authorization/ACL/runtime admission, or blocking obligations; E8.1.T5 attested promotion `promote_github_attested_proposal_candidate_v1` reuses it for a removed allowlist bot, revoked workload session, digest failure, moved head or blocking obligation; returned to the promotion caller before activation |
...
| `governance.attestation_not_current` | unreleased (E8.1.T5) | Cloud `evaluate_github_attestation_gate_decision_v1` raises this fixed refusal (SQLSTATE `42501`) when `gate_facts_with_github_attestation_v1` does not report the referenced attestation `valid_current` under the exact proposal, policy, identity and observation bindings, including an attestation referenced while the current approval-source policy selects `native`; checked after `gate.policy_missing` and returned to the attested evaluator caller before any gate decision is written. Separately, the gate-decision insert trigger raises the same code for a non-attested decision that references a native approval or a break-glass emergency receipt while the current approval-source policy selects `github_attestation`, and native promotion raises it under that mode; that write is refused, so no decision or promotion is retained. These are blocking refusals, not `adoc.gate_result.v0` reasons, and never replace shipped `gate.approval_missing`: a head that references no attestation, native approval or break-glass receipt (no review yet, or only a rejected attempt) reaches none of them and publishes a decision with `gate.approval_missing`, the `approval_required` reason in both modes; no new effect is authorized |
...
| `gate.policy_missing` | unreleased (E8.1.T4) | The E8.1.T4 approval-source mode mutation raises this fixed refusal (SQLSTATE `42501`) when asked to select `github_attestation` for a repository with no `repository_gate_policies` row; that governed write is refused, so no mode revision, receipt or audit event is retained. Separately, the E8.1.T5 evaluator `evaluate_github_attestation_gate_decision_v1` raises the same code for the same absence as a fail-closed backstop; this departs deliberately from the shipped `adoc.gate_result.v0` absent-mode `advisory` default for attested evaluation only, because an attested approval needs an explicit gate policy to evaluate against, while native evaluation never reaches this evaluator and still derives `advisory`, so no repository silently gains a blocking gate; checked first, before attestation facts or `governance.attestation_not_current`; returned to the attested evaluator caller before any gate decision is written, so it is a blocking refusal, not an `adoc.gate_result.v0` reason, and never replaces shipped `gate.approval_missing`; admission fails closed rather than falling back to native or an older policy. Gate-policy revisions are append-only (Cloud `20260905000000_create_gate_evaluator.sql` refuses update, delete and truncate), so valid configuration never reaches the evaluator refusal |
...
| `ingest.envelope_version_unsupported` | planned (E4.4) | assessment submission names an unknown or superseded exact operation/envelope version; remediation names the accepted version; E8.2: a proposal delivery report whose `schema_version` is not `agentdoc.cloud.proposal_delivery.v0` is refused with the supported versions, and this is also the remediation a `pr`-mode proposal delivery response names beside the recorded `delivery.reference_missing` refusal when the observed `adoc.git_proposal_references` block carries an unknown version |
| `ingest.duplicate_delivery` | planned (E4.6) | a byte-identical replay of an already-complete assessment delivery (E4.6), or a proposal command whose proposal-set digest is already recorded with the same patch set (E5.1; bindings and placement metadata may differ), is acknowledged without another ingestion record, activation event, or latest-state mutation; redelivery may complete an explicitly partial record; E8.2: a replayed proposal delivery report under the same idempotency key returns `200` with this code, the original attempt's `delivery_id` and its code as `original_code` (`null` only when it was accepted), and creates no new attempt, and a proposal delivery report naming a commit already accepted for the same proposal version is retained as a refused attempt and creates no second association |
...
| `ingest.digest_mismatch` | planned (E4.6) | a claimed assessment or receipt digest does not match the submitted envelope bytes; the attempt is recorded and the delivery rejected; E8.2: a proposal delivery report whose repository or change-request identity, assessed head, receipt, proposal-set or object digests, or supplied reference-block bytes disagree with stored evidence or the provider observation, or whose observed changed-path set differs from or exceeds the proposal's bound patch paths, or whose provider answer is definitive (a `404` after one retry, or a body over the byte bound; `observation.provider_reason`, E8.2.F1), is retained as a refused attempt with no association |
...
| `connect.installation_mode_conflict` | planned (E8.1.T4, activation slice A0) | one installation, one approval-source mode, across Workspaces (`docs/plans/E8.1-QUALIFICATION-PLAN.md` D-13): both `record_github_approval_source_policy` and `connect_github_repository_installation` raise this fixed refusal (SQLSTATE `42501`) when the requested mode change or new binding would leave one installation serving repositories in different committed modes; the governed write is refused before any revision, binding or audit row is retained, the connection stays healthy and no grant is involved, and, because binding rows are immutable, the remedy is to record the mode on each repository before binding it or to use a new App installation (documented limitation, plan R-8); the refusal is symmetric, so rollback to `native` is per installation and equally refused while another bound repository stays attested |
...
| `delivery.reference_missing` | planned (E8.2.T2) | Cloud rejects the delivery association and retains the diagnostic on the proposal record when, for a `pr`-mode delivery, the Action-owned `adoc.git_proposal_references.v0` block observed on the provider knowledge PR body is absent, incomplete, duplicated or carries an unknown key or version; the accepted Proposal Record version is never mutated or repaired |
| `delivery.reference_stale` | planned (E8.2.T2) | Cloud rejects the delivery association and retains the diagnostic on the proposal record when the reference block is bound to a superseded assessment head: the assessment ingestion is no longer current at admission, or the provider-observed heads or delivered-commit parent differ from the mode's currentness predicate; the original binding is untouched and never silently repaired |
| `delivery.provider_unavailable` | planned (E8.2.T2) | Cloud's server-side provider observation for an `agentdoc.cloud.proposal_delivery.v0` report is transiently unavailable or partial (5xx, rate limit, timeout, credentials); retryable `503` returned before request registration, so nothing is persisted and the identical retry can still succeed; a definitive answer (a `404` after one retry, or a body over the byte bound) is not this code and is retained as `ingest.digest_mismatch` with `observation.provider_reason` (E8.2.F1); distinct from the E6.5 `writeback.provider_unavailable` |
...
## Attestation codes — unreleased, owner `cloud`

The canonical bot-attestation code family root (RT-21, E0.3.T3). The Action never mints its own bot-attestation family: its check surface wraps `attestation.bot_approver_rejected` via the registered `action.attestation_bot_rejected` row above. E8.1.T1 registers the sibling codes `attestation.binding_mismatch` and `attestation.requirements_unmet` beside it; E8.1.T2 registers the installation permission-health code `attestation.permission_profile_mismatch`, which is not an attestation result code. A row is registered `planned` with its owning slice and flips to `unreleased` when that slice's implementation and tests land; the whole E8.1 family flipped on 2026-09-25 (cloud#169 merged as `e8e9c276`), so every row below is `unreleased`. Rows are never re-registered.

<!-- registry:attestation-codes -->
| code | status | planned by | meaning |
| --- | --- | --- | --- |
| `attestation.bot_approver_rejected` | unreleased | E8.1.T1 | bot/service/agent/workload approver rejected by default for approval attestation; an exact entry in E8.1.T4's Workspace bot allowlist may lift only this identity-type rejection, and only in a `github_attestation`-mode repository |
| `attestation.binding_mismatch` | unreleased | E8.1.T1 | authenticated review evidence does not exact-match the retained Workspace, repository, pull request, head SHA, Proposal Set digest or exposure binding |
| `attestation.requirements_unmet` | unreleased | E8.1.T1 | one or more required review, CODEOWNERS, check, branch-protection, authorization or merge predicates is false, unknown or unreadable |
| `attestation.permission_profile_mismatch` | unreleased | E8.1.T2 | installation permission-health observation code, recorded only by Cloud's `record_github_approval_attestation_permission_observation` when the observed GitHub installation permission profile cannot support authenticated review, protection or rule reads; an unhealthy observation admits no attestation. Outside the closed three-code `AttestationFailure` union and the four-status projection, never rendered by the Action; permission-denied attestation results use `attestation.requirements_unmet` |
<!-- /registry:attestation-codes -->
