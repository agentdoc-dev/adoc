...
| `agentdoc.cloud.candidate_authority.v0` | cloud | E4.3 | immutable candidate provenance binding the exact managed candidate version ID and content digest, authenticated submitting principal, exact policy receipt, policy effect ordinal and transaction ID, strictly later candidate-operation ordinal and transaction ID, authority mode, and proposal origin; proposal origin is server-derived from authenticated ingress and principal class, never caller-supplied; connector-origin candidates require the exact connector Source Assertion, while AgentDoc-origin candidates carry no connector Source Assertion |
