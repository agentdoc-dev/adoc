...
| `agentdoc.cloud.github_approval_attestation_attempt.v0` | cloud | E8.1.T3 | canonical rejected-attempt record binding the exact Workspace, proposal exposure, reviewer identity link, closed status/code pair, request digest and immutable attempt ID; its digest covers the rejection outcome so display cannot substitute an uncovered status or code. An unlinked GitHub reviewer produces no canonical attempt and no status projection; admission returns `attestation.requirements_unmet` |
| `agentdoc.cloud.github_approval_attestation_status.v0` | cloud | E8.1.T3 | display-only projection binding one retained attestation or attempt to the exact current Workspace/repository/pull request/head SHA, Proposal Set version/digest, and the ID/digest of the gate decision published for that head (it exists with or without an approval); only the four closed attestation statuses are accepted, and the projection grants no approval authority |
