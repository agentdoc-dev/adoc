...
| `delivery.provider_unavailable` | planned (E8.2.T2) | Cloud's server-side provider observation for an `agentdoc.cloud.proposal_delivery.v0` report is unavailable or partial; retryable `503` returned before request registration, so nothing is persisted and the identical retry can still succeed; distinct from the E6.5 `writeback.provider_unavailable` |
