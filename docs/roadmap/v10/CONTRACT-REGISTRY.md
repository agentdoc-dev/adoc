...
| `adoc.semantic_executor_request.v0` | shipped | adoc 0.4.x | Action v2 Claude/Codex/generic/human adapters; customer-hosted/local executors | exact-match reader with additive human-review bindings; one request shape embeds an integrity-validated ready semantic context, closed adapter and endpoint classes, 60–3600 second timeout, prompt contract, and exact executor/model/config/task/prompt digests; the prompt digest is SHA-256 over compact canonical JSON containing exactly its contract version and instructions; human uses the identical boundary with closed `human` adapter/provider/endpoint bindings; legacy requests without Principal bindings remain valid but cannot complete with review authority, while authoritative completion requires and rechecks exact trusted reviewing/requesting Principal IDs |
