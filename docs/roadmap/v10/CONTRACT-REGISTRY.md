...
| `governance.proposal_invalid` | planned (E5.1) | Cloud proposal-command route rejects an `adoc.proposal.v0` payload that fails the pinned schema, whose patch or proposal-set digests do not re-derive from the embedded patch bytes, whose patches are out of canonical order, or that would mint authority (ADR-0053 §2–§3, ADR-0054 §3 floors); remediation names the failing derivation |
| `governance.proposal_conflict` | planned (E5.1) | a proposal command names a proposal-set digest already recorded in the Workspace with different record bytes; the stored record is immutable and the delivery is rejected, never merged |
...
| `ingest.duplicate_delivery` | planned (E4.6) | a byte-identical replay of an already-complete assessment delivery (E4.6) or proposal command (E5.1) is acknowledged without another ingestion record, activation event, or latest-state mutation; redelivery may complete an explicitly partial record |
