...
| `approval.proposal_hash_mismatch` | planned (E5.2) | approval command does not bind the exact current proposal digest, so no approval record is created; distinct from `gate.proposal_hash_mismatch`, which evaluates an already-recorded approval at gate time, not a rejected command |
