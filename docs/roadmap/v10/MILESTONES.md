...
4. `E4.4.T4` — Remaining operation contracts registered as versioned schemas with round-trip fixtures: `ingestion_result`, `repository_config`, `work_request`, `work_result`, `gate_decision`, `proposal_command`, `approval_command`, `authorization_decision`, `migration_request`, `migration_receipt`, `egress_policy` (all `agentdoc.cloud.*.v0`); each entered in the E0.3 registry before its route exists.
**Acceptance:**
