...
3. `E2.3.T3` — Unlink/revocation: revokes future use, preserves history; failing test: actions before unlink retain original identity attribution.
...
- An external membership observation confers only while its preserved E2.3 identity link was active at the decision evaluation time; later unlink preserves historical replay but revokes future use (exit gate).
