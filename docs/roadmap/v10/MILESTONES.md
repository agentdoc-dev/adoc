...
- A grant may cite only the binding-mode epoch in effect at its evaluation time; an observation whose `effective_at` falls in a superseded epoch, or whose `observed_at` falls outside the event or synchronization run identified by `source_event_id`, confers nothing (exit gate).
