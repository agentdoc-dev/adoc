...
2. `E2.4.T2` — External binding with exact modes `authoritative_sync` / `additive_sync` / `suggestion_only` / `disabled`; failing fixtures:
   ...
   - An observation confers nothing when its `effective_at` falls in a superseded epoch, its `observed_at` falls outside the event or run identified by `source_event_id`, its `effective_at` does not equal the commit or completion instant of the event or run identified by that same `source_event_id`, its retained binding or binding-owned group differs from the enclosing grant, its `source_kind` differs from the source kind recorded on the retained binding, or its external identity link was not continuously active from `observed_at` through evaluation time according to retained link and unlink instants.
   - A delayed or reordered positive event without a confirming current-state source read confers nothing, and only the transition sweep that opened the epoch may carry an `observed_at` before that epoch.
   - A later reconfiguration supersedes an in-flight request so its completed sweep records no epoch and cannot undo `disabled`; changing to a grant-conferring mode — including re-enabling — keeps the prior epoch and grants on resync failure while surfacing the requested target, pending state, and failure, then restores an unchanged member's grant only from a source-read observation made effective in the new epoch on success.
   - Linking or relinking an external identity completes the link and records a pending current-state membership read for that principal against every grant-conferring binding of the linked source kind. Each binding resolves independently: each binding read and its outcome are recorded and operator-visible, a successful read must cite the new link, and a failed read is surfaced with an explicit operator retry; the fixture covers total and partial failure without discarding successful sibling observations.
   - Claim-only `oidc_group` treats a freshly issued and verified ID token as its current-state read, carries no observation across the new epoch, and restores each principal only on later authentication. The decision principal `identity_session_id` must equal the identity session retained by `source_event_id`, and the observation confers no later than the cited identity session's expiry.
...
- Linking or relinking an external identity completes the link and records a pending current-state membership read for that principal against every grant-conferring binding of the linked source kind. Each binding resolves independently: each binding read and its outcome are recorded and operator-visible, successful reads cite the new link, and a failed read is surfaced with an explicit operator retry while successful sibling observations remain usable. Claim-only `oidc_group` instead recovers at the next authentication (exit gate).
- Claim-only `oidc_group` uses a freshly issued and verified ID token as its current-state read; its retained identity-session event carries token issuance, validation/ingestion-commit, and session-expiry instants, and the decision principal `identity_session_id` must equal the identity session retained by `source_event_id`. The observation confers only for that matching session and no later than the cited identity session's expiry; it has no out-of-band sweep, provider claim revocation takes effect per session at the next authentication within the E2.3.T5 session-lifetime bound, and grant-conferring reconfiguration restores each principal only on a later authentication in the new epoch (exit gate).
