...
2. `E2.4.T2` — External binding with exact modes `authoritative_sync` / `additive_sync` / `suggestion_only` / `disabled`; failing test: `suggestion_only` changes nothing without human action, reconfiguring a binding to `disabled` or resyncing it preserves the prior mode epoch and cited membership evidence so a decision recorded under it still replays, an observation whose `effective_at` falls in a superseded epoch, whose `observed_at` falls outside the event or run identified by `source_event_id`, whose `effective_at` does not equal the commit or completion instant of the event or run identified by that same `source_event_id`, whose retained binding or binding-owned group differs from the enclosing grant, whose `source_kind` differs from the source kind recorded on the retained binding, or whose external identity link was not continuously active from `observed_at` through evaluation time according to retained link and unlink instants confers nothing, a delayed or reordered positive event without a confirming current-state source read confers nothing, only the transition sweep that opened the epoch may carry an `observed_at` before that epoch, a later reconfiguration supersedes an in-flight request so its completed sweep records no epoch and cannot undo `disabled`, and reconfiguring to a grant-conferring mode — including re-enabling — keeps the prior epoch and grants on resync failure while surfacing the requested target, pending state, and failure before restoring an unchanged member's grant from a source-read observation made effective in the new epoch on success; Linking or relinking an external identity records a current-state membership read for that principal against every grant-conferring binding of the linked source, and the fixture rejects reusing the old-link observation after relink; claim-only `oidc_group` treats a freshly issued and verified ID token as its current-state read, binds the observation to the exact identity session no later than the cited identity session's expiry, carries no observation across the new epoch, and restores each principal only on later authentication. Claim-only `oidc_group` instead recovers at the next authentication.
...
- A delayed or reordered positive event confers nothing without a current-state source read that still confirms membership; only the transition sweep that opened the epoch may carry an `observed_at` before that epoch, while every other observation must be read within the epoch it can confer in (exit gate).
- An external membership observation confers only when its preserved E2.3 identity link, including retained link and unlink instants, was continuously active from `observed_at` through evaluation time; later unlink preserves historical replay but the same observation stays revoked after a subsequent relink (exit gate).
- Linking or relinking an external identity records a current-state membership read for that principal against every grant-conferring binding of the linked source; only an observation citing the new link can restore event-driven membership. Claim-only `oidc_group` instead recovers at the next authentication (exit gate).
- Claim-only `oidc_group` uses a freshly issued and verified ID token as its current-state read; its retained identity-session event carries token issuance, validation/ingestion-commit, and session-expiry instants, and its observation confers no later than the cited identity session's expiry. It has no out-of-band sweep, provider claim revocation takes effect at the next authentication within the E2.3.T5 session-lifetime bound, and grant-conferring reconfiguration restores each principal only on a later authentication in the new epoch (exit gate).
- Reconfiguration to `suggestion_only` or `disabled` takes effect immediately; a change to a grant-conferring mode remains under the prior epoch until resync completes and records current source membership in the new epoch, with pending state and failure visible to operators; any later reconfiguration supersedes an in-flight request, whose completion records no epoch and confers nothing (exit gate).
...
- External binding records and complete effective mode histories, membership observations, and their source events or synchronization runs remain retained for every decision that cites them after observed-membership revocation, resync, reconfiguration, or disablement, with each retained connector source event carrying its current-state read and ingestion-commit instants and each retained synchronization run carrying its start and completion instants; a requested reconfiguration, its request instant, and its resynchronization outcome and outcome instant are retained for every decision recorded while it was pending. Replay verifies that the observation's retained binding equals the enclosing binding_id, that binding's group equals the enclosing group id, and that source_kind equals the source kind recorded on the retained binding (exit gate).
