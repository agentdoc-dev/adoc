...
- Every external membership observation retains `fresh_until`, derived from `observed_at` under the versioned freshness policy retained by the exact binding. Replay permits it to confer only when `evaluation_time` does not exceed `fresh_until`; connector unavailability cannot extend the deadline, and claim-only `oidc_group` is additionally capped at the cited identity session's expiry (exit gate).
