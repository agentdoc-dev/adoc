...
2. `E2.4.T2` — External binding with exact modes `authoritative_sync` / `additive_sync` / `suggestion_only` / `disabled`; failing fixtures:
   ...
   - Every decision records required `membership_evidence`. A group-bearing grant requires `current`; an unresolved manual-membership lifecycle, expired external observation, unavailable connector read, pending or failed suspended link read, or empty new grant-conferring epoch awaiting its first resynchronization or `oidc_group` authentication records `insufficient_context` and reason `membership_evidence_unavailable` with a null basis, yielding `insufficient_context` for consequential decisions or `deny` otherwise. `no_grant` with `current` retains `membership_absence_evidence` for every relevant manual group or external binding, including exact negative observation and source-run provenance for external membership; `no_grant` with `not_applicable` carries no absence evidence.
...
- Every decision records required `membership_evidence`; a group-bearing grant requires `current`. An unresolved manual-membership lifecycle, expired external observation, unavailable connector read, pending or failed suspended link read, or empty new grant-conferring epoch awaiting its first resynchronization or `oidc_group` authentication records `insufficient_context` and reason `membership_evidence_unavailable` with a null basis, yielding `insufficient_context` for consequential decisions or `deny` otherwise. `no_grant` with `current` retains `membership_absence_evidence` for every relevant manual group or external binding, including exact negative observation and source-run provenance for external membership; `no_grant` with `not_applicable` carries none (exit gate).
