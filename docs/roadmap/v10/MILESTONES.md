...
2. `E2.4.T2` — External binding with exact modes `authoritative_sync` / `additive_sync` / `suggestion_only` / `disabled`; failing test: `suggestion_only` changes nothing without human action, and reconfiguring a binding to `disabled` or resyncing it preserves the prior mode epoch and cited membership evidence so a decision recorded under it still replays.
