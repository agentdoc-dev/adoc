...
- Claim-only `oidc_group` uses a freshly issued and verified ID token as its current-state read, has no out-of-band sweep, and after grant-conferring reconfiguration restores each principal only on a later authentication in the new epoch (exit gate).
