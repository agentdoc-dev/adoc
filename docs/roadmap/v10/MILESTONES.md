...
2. `E2.4.T2` — External binding with exact modes `authoritative_sync` / `additive_sync` / `suggestion_only` / `disabled`; failing fixtures:
   ...
   - The `oidc_group` source kind is claim-only in V1 and valid only for a human principal; a freshly issued and verified ID token is its current-state read, while a token requiring out-of-band group lookup produces no observation. It carries no observation across the new epoch and restores each principal only on later authentication. The decision principal `identity_session_id` must equal the identity session retained by `source_event_id`, and the observation confers no later than the cited identity session's expiry.
