...
2. `E2.4.T2` — External binding with exact modes `authoritative_sync` / `additive_sync` / `suggestion_only` / `disabled`; failing fixtures:
   ...
   - Event polarity only triggers a current-state read; the read result determines observation polarity. A positive event whose read confirms absence records a negative observation and no positive observation. A removal event whose read still shows membership records a positive observation and no negative observation. Its `fresh_until` is capped at the deadline of the latest prior positive observation for the same group, binding, and identity link when one exists; when no prior positive observation exists, the ordinary historical-policy deadline applies. A cap at or before `effective_at` makes the observation born inert and the input records `observation_expired`. The event-versus-read contradiction is retained and operator-visible, and the binding's next scheduled resynchronization bounds revocation propagation. Only the transition sweep that opened the epoch may carry an `observed_at` before that epoch.
   - Every decision records required `membership_evidence`. A group-bearing grant forbids `not_applicable`: it records `current` when every relevant membership input was established, or `insufficient_context` when another input remains unresolved. Every `insufficient_context` status retains `membership_unavailability_evidence` for every unresolved relevant input, identifying the stable group id and, unless reason is `evaluation_time_invalid`, its evaluation-time name, plus immutable expiry, failure, or pending-state provenance. External evidence additionally identifies the source kind, identity link, and binding-mode epoch actually in effect; `epoch_observation_pending` may cite the prior `suggestion_only` or `disabled` epoch and its retained pending transition request rather than inventing a grant epoch. Registered states are `lifecycle_unavailable` for manual input, `observation_expired` for connector or OIDC input, connector-only `connector_read_failed` / `link_read_pending` / `link_read_failed` / `epoch_observation_pending`, and OIDC-only `oidc_authentication_pending`. Any OIDC unavailable input is human-only. OIDC `observation_expired` carries the expired observation's historical session on that evidence entry, while an `oidc_authentication_pending` entry carries none of its own even if sibling resolved OIDC evidence independently requires an envelope session. When the scoped-grants stage owns the outcome, unresolved membership records reason `membership_evidence_unavailable` with a null basis, yielding `insufficient_context` for consequential decisions or `deny` otherwise; earlier identity, evaluation-time, hard-deny, or source-ACL outcomes and a decisive `explicit_deny` retain precedence. `no_grant` with `current` retains `membership_absence_evidence` for every relevant confirmed absence, including exact negative observation and source-run provenance for external membership. A group-bearing grant is a necessary but not sufficient condition for an empty array; the array is empty only when no relevant membership was established absent. `no_grant` with `not_applicable` carries no absence evidence.
   - Every observation carries `fresh_until`, derived from `observed_at` under the versioned freshness policy retained by the exact binding. The binding retains its complete effective membership-freshness policy history; this membership-freshness policy is distinct from the connector source-ACL policy owned by E2.6. Replay resolves the historical version, recomputes the deadline, and requires `fresh_until` to equal that recomputation except that a positive removal-event read is capped at the latest prior positive deadline when one exists; with no prior positive observation, the ordinary recomputation governs. At authorization time the effective deadline is the shorter of retained `fresh_until` and the evaluation-time policy recomputation, so a shorter policy caps existing observations immediately and a longer policy never revives or extends them. An uncapped observation's effective deadline must follow `effective_at`; a capped observation whose deadline does not is born inert, retained for replay, and recorded as `observation_expired`. A period of connector unavailability cannot extend the deadline, and claim-only `oidc_group` is additionally capped at the cited identity session's expiry. A run whose observations cannot satisfy the uncapped ordering is rejected as a freshness-policy misconfiguration; `no_grant` remains reserved for a confirmed absence of an applicable grant.
   ...
   - A later reconfiguration supersedes an in-flight request so its completed sweep records no epoch and cannot undo `disabled`; a failed transition between grant-conferring modes keeps the prior epoch and still-fresh grants, while a failed re-enable remains non-granting under the prior `suggestion_only` or `disabled` epoch. Success restores an unchanged member's grant only from a source-read observation made effective in the new epoch. Claim-only `oidc_group` instead opens a grant-conferring epoch after provider-configuration validation and restores each principal only through later authentication.
...
3. `E2.4.T3` — Revocation propagation: failing fixture: external revocation under `authoritative_sync` removes derived membership while retaining the binding, membership observation, and source event so a decision recorded before revocation still replays; when an event-triggered read still reports membership, current state remains positive but `fresh_until` is capped at the latest prior positive deadline when one exists, so the revocation signal and its redelivery cannot renew prior membership; a cap at or before `effective_at` is born inert and records `observation_expired`, the contradiction is retained and operator-visible, and the next scheduled resynchronization bounds propagation; manual member survives only where group policy permits.
...
- `authoritative_sync` revocation removes derived membership; when its event-triggered read still reports membership, current state remains positive but `fresh_until` is capped at the latest prior positive deadline when one exists so the revocation signal and its redelivery cannot renew prior membership; a cap at or before `effective_at` is born inert and records `observation_expired`, the contradiction is retained and operator-visible, and the next scheduled resynchronization bounds propagation (exit gate). `disabled` ignores observations entirely.
...
- Every decision records required `membership_evidence`; a group-bearing grant forbids `not_applicable`, recording `current` when every relevant membership input was established or `insufficient_context` when another input remains unresolved. Every `insufficient_context` status retains `membership_unavailability_evidence` for every unresolved relevant input, identifying the stable group id and, unless reason is `evaluation_time_invalid`, its evaluation-time name, plus immutable expiry, failure, or pending-state provenance. External evidence additionally identifies the source kind, identity link, and binding-mode epoch actually in effect; `epoch_observation_pending` may cite the prior `suggestion_only` or `disabled` epoch and its retained pending transition request rather than inventing a grant epoch. Registered states are `lifecycle_unavailable` for manual input, `observation_expired` for connector or OIDC input, connector-only `connector_read_failed` / `link_read_pending` / `link_read_failed` / `epoch_observation_pending`, and OIDC-only `oidc_authentication_pending`. Any OIDC unavailable input is human-only. OIDC `observation_expired` carries the expired observation's historical session on that evidence entry, while an `oidc_authentication_pending` entry carries none of its own even if sibling resolved OIDC evidence independently requires an envelope session. When the scoped-grants stage owns the outcome, unresolved membership records reason `membership_evidence_unavailable` with a null basis, yielding `insufficient_context` for consequential decisions or `deny` otherwise; earlier identity, evaluation-time, hard-deny, or source-ACL outcomes and a decisive `explicit_deny` retain precedence. `no_grant` with `current` retains `membership_absence_evidence` for every relevant confirmed absence, including exact negative observation and source-run provenance for external membership. A group-bearing grant is a necessary but not sufficient condition for an empty array; the array is empty only when no relevant membership was established absent. `no_grant` with `not_applicable` carries none (exit gate).
- Every external membership observation retains `fresh_until`, derived from `observed_at` under the versioned freshness policy retained by the exact binding. The binding retains its complete effective membership-freshness policy history; this membership-freshness policy is distinct from the connector source-ACL policy owned by E2.6. Replay resolves the historical version, recomputes the deadline, and requires `fresh_until` to equal that recomputation except that a positive removal-event read is capped at the latest prior positive deadline when one exists; with no prior positive observation, the ordinary recomputation governs. At authorization time the effective deadline is the shorter of retained `fresh_until` and the evaluation-time policy recomputation, so a shorter policy caps existing observations immediately and a longer policy never revives or extends them. An uncapped observation's effective deadline must follow `effective_at`; a capped observation whose deadline does not is born inert, retained for replay, and recorded as `observation_expired`. A period of connector unavailability cannot extend the deadline, and claim-only `oidc_group` is additionally capped at the cited identity session's expiry. A run whose observations cannot satisfy the uncapped ordering is rejected as a freshness-policy misconfiguration; `no_grant` remains reserved for a confirmed absence of an applicable grant (exit gate).
...
- Claim-only `oidc_group` instead opens a grant-conferring epoch after provider-configuration validation and restores each principal only through a later authentication in that epoch, never through a connector resynchronization (exit gate).
