...
- Every decision records required `membership_evidence`; a group-bearing grant forbids `not_applicable`, recording `current` when every relevant membership input was established or `insufficient_context` when another input remains unresolved. When the scoped-grants stage owns the outcome, unresolved membership records reason `membership_evidence_unavailable` with a null basis, yielding `insufficient_context` for consequential decisions or `deny` otherwise; earlier identity, evaluation-time, hard-deny, or source-ACL outcomes and a decisive `explicit_deny` retain precedence. `no_grant` with `current` retains `membership_absence_evidence` for every relevant confirmed absence, including exact negative observation and source-run provenance for external membership. A group-bearing grant is a necessary but not sufficient condition for an empty array; the array is empty only when no relevant membership was established absent. `no_grant` with `not_applicable` carries none (exit gate).
