...
2. `E2.4.T2` — External binding with exact modes `authoritative_sync` / `additive_sync` / `suggestion_only` / `disabled`; failing fixtures:
   ...
   - Every observation carries `fresh_until`, derived from `observed_at` under the versioned freshness policy retained by the exact binding. The binding retains its complete effective membership-freshness policy history; this membership-freshness policy is distinct from the connector source-ACL policy owned by E2.6. The observation confers only when `evaluation_time` does not exceed `fresh_until`, `fresh_until` must follow `effective_at`, connector unavailability cannot extend the deadline, and claim-only `oidc_group` is additionally capped at the cited identity session's expiry. A run that cannot complete before the deadline is rejected as a freshness-policy misconfiguration.
   - Every grant-conferring connector-read binding resynchronizes on a schedule shorter than its freshness window; an unchanged member receives a replacement observation before expiry, while a failed scheduled run is recorded and surfaced with an explicit operator retry without extending the prior deadline.
...
- Every external membership observation retains `fresh_until`, derived from `observed_at` under the versioned freshness policy retained by the exact binding. The binding retains its complete effective membership-freshness policy history; this membership-freshness policy is distinct from the connector source-ACL policy owned by E2.6. Replay permits the observation to confer only when `evaluation_time` does not exceed `fresh_until`; `fresh_until` must follow `effective_at`, connector unavailability cannot extend the deadline, and claim-only `oidc_group` is additionally capped at the cited identity session's expiry. A run that cannot complete before the deadline is rejected as a freshness-policy misconfiguration (exit gate).
- Every grant-conferring connector-read binding resynchronizes on a schedule shorter than its freshness window, replacing unchanged positive observations before expiry. A failed scheduled resynchronization is recorded and surfaced with an explicit operator retry without extending the prior deadline (exit gate).
