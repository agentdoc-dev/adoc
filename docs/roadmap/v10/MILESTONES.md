...
- `authoritative_sync` revocation removes derived membership; when its event-triggered read still reports membership, current state remains positive but `fresh_until` is capped at the latest prior deadline so the revocation signal and its redelivery cannot renew membership, the contradiction is retained and operator-visible, and the next scheduled resynchronization bounds propagation (exit gate). `disabled` ignores observations entirely.
