...
- `authoritative_sync` revocation removes derived membership; when its event-triggered read still reports membership, current state remains positive, the contradiction is retained and operator-visible, and the next scheduled resynchronization bounds propagation (exit gate). `disabled` ignores observations entirely.
