...
- `crates/adoc-core/src/domain/diagnostic.rs` — the kind's `DiagnosticCode` variants. Field-level rules live inside `build_from_parsed`; cross-object rules get a new file in `crates/adoc-core/src/language/validate/` wired into `validate/mod.rs` (patterns: `policy_active_approval.rs`, `evidence_quality.rs`).
...
- `crates/adoc-core/src/language/render/html.rs` — a `render_<kind>` fn on the `render_knowledge_object` dispatch.
...
- New files: `crates/adoc-core/src/domain/knowledge_object/api.rs`, `crates/adoc-core/src/domain/value_objects/http_method.rs`, `crates/adoc-core/src/language/validate/api_verified_evidence.rs`, `crates/adoc-cli/tests/api_cli.rs` — plus the shared-checklist touches.
...
- HTML renders an endpoint signature header — method badge plus path in code style — above the prose body (`render_api` in `language/render/html.rs`).
...
- New files: `crates/adoc-core/src/domain/knowledge_object/question.rs`, `crates/adoc-core/src/language/validate/question_resolved_by.rs`, `crates/adoc-cli/tests/question_cli.rs` — plus the shared-checklist touches.
...
- New files: `crates/adoc-core/src/domain/knowledge_object/task.rs`, `crates/adoc-core/src/language/validate/task_overdue.rs`, `crates/adoc-cli/tests/task_cli.rs` — plus the shared-checklist touches.
...
- Update the exact-match diagnostic budget (0 errors, 6 warnings — the V5.10 five plus `task.overdue`) and the per-kind count table in [expanded-pilot.md](../guides/expanded-pilot.md). Today's table reads: claim 8, decision 1, glossary 2, constraint 1, procedure 1, example 2, policy 1, agent_instruction 1, contradiction 1, source 2 — 20 KO nodes, 12 page nodes; this slice grows it by at least six objects across four new kinds. Budget table and `crates/adoc-cli/tests/expanded_pilot.rs` exact-match assertions update in the same commit, per the pilot's standing procedure. The expected budget after this slice ([expanded-pilot.md](../guides/expanded-pilot.md) remains the canonical copy; this is a preview):
...
- [v1-retrieval.md](../design/v1-retrieval.md) maintenance docs updated; the V4 retrieval migration hint retired or downgraded now that prose is searchable — the hint's trigger (`prose_block_count > 0`, zero KOs, search requested) now describes a working configuration, not a dead end.
...
- The workflow list and prose tool coverage in `docs/guides/mcp-agent-gateway.md` omit `adoc_stale`, `adoc_contradictions`, `adoc_impacted_by`, `adoc_diff`, and `adoc_review` — the last two have been missing since V3, so this lag predates V6.
...
- **V4.5 Markdown migration** — *discharged: shipped as V8.1 ([ROADMAP-V8.md](ROADMAP-V8.md))*. This entry gated `adoc migrate` on measured compat-mode friction; the V8 cycle resolved the gate structurally (an external pilot cannot run without an import path) and retired the "V4.5" placeholder number. `adoc migrate`, the `adoc.migrate.report.v0` envelope, suggested typed blocks, and reversible export shipped across V8.1.1–V8.1.4, closing PRD MVP must-have #18.
- **External-corpus pilot (V7.2 follow-up)** — *thread picked up: this is the V8.2 milestone ([ROADMAP-V8.md](ROADMAP-V8.md))*. The original entry: a second pilot on a repository the maintainer does not own. Dogfooding `docs/` discharges §50.1 #13/#14 as written, but the stronger proof is a corpus with no home-field advantage; V8.2 runs 2–3 external design-partner pilots under the V7.2 discipline.
