...
- V8.1 Markdown migration ([ROADMAP-V8.md](ROADMAP-V8.md), ADR-0043): `adoc migrate` — lossless `.md` → prose-mode `.adoc` import (graph-semantic losslessness held over the Markdown Pilot; raw HTML and unrecognized extensions quarantined in fenced carriers, each backed 1:1 by a `migrate.*` diagnostic), the `adoc.migrate.report.v0` envelope with exact reconcilable counts and an MCP schema resource, §28.4 suggested typed blocks (rules-not-weights, never auto-typed — held by a property test), and reversible export (`adoc migrate --export`, byte-idempotent round trip modulo the ADR-0043 §5 closed normalization set; typed-block pages refuse with `migrate.export_typed_blocks_present`). Symmetric dry-run/`--write`/`--force` semantics with committed-clean refusal both ways. Retires the "V4.5" placeholder; closes PRD §28, §9.6, §50.1 #12, and MVP Must-Have #18 — the last unfinished MVP engineering item.
...
- V7.2 dogfood evidence remains governed by accepted ADR-0042 and its `docs/pilots/dogfood/report.md`; V10.8.1 runs that mandatory track, alongside the V8.2 external-pilot debt, against the ledger frozen at V10.1.7 rather than claiming either complete retroactively.
