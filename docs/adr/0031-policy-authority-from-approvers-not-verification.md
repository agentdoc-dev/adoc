...
**Required-field validation is aggregate-owned; the date-dependent active rule is an infrastructure rule.** Presence/format/grammar checks (`schema.policy_missing_status|owner|approved_by|effective_at|body`, `schema.policy_invalid_effective_at|review_interval`, and the shared `schema.invalid_status`) live in `policy.rs`'s fallible constructor, mirroring V5.1–V5.3 and keeping `infrastructure/validate/objects/` deferred (ADR-0030). The one genuinely clock-dependent invariant — an `active` policy must have `effective_at <= today` — lives in `PolicyActiveApproval`, a `ValidationRule` under `infrastructure/validate/` that receives `today: NaiveDate` from the compile pipeline exactly as `KnowledgeObjectLifecycle` does, and emits `schema.policy_future_effective_at`. `EffectiveDate` is a typed value object wrapping `chrono::NaiveDate` (canonical `YYYY-MM-DD`), so the date is parsed once at construction and the rule only compares.
