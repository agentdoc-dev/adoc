# Launch readiness: local candidate

Verified 2026-09-18 on `feat/hn-launch-readiness`. This records a locally tested
...
| LA-2: Installation | Documented source build includes CLI and MCP; offline/real-model smoke and actual Claude Code status/retrieval pass | Four native Linux/macOS packaging jobs are configured, but their hosted runs are still required before release |
...
| LA-6: Contribution | CONTRIBUTING.md, issue forms, setup/check/help instructions added | No live external contributor PR was submitted in this audit |
...
| LA-9: Fork review | Optional secret-backed Claude review is restricted to same-repository human PRs; ordinary CI remains available to forks | Predicate/lint verification, not a live fork-PR run |
...
The final code snapshot passed all 2,966 tests across 98 suites (3 ignored),
...
amendment. No actionable review finding remains.
