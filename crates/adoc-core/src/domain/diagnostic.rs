...
diagnostic_codes! {
    ...
    /// Object ID grammar, a non-semantic reason, or a create/edit sequence
    /// outside ADR-0054 §5.
    ProposalRecordPatchInvalid = "proposal_record.patch_invalid" =>
        "Use intrinsically valid canonical patches with Object ID targets, semantic-text reasons, and matching create placement; serialize each as sorted compact JSON with one trailing newline; create a target once or edit it with at most one update_fields then at most one replace_body.";
