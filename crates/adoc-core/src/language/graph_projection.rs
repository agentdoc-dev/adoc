...
impl GraphProjection {
    ...
    pub(crate) fn build_for_date(
        ...
        GraphArtifactDocument {
            schema_version: GRAPH_ARTIFACT_SCHEMA_VERSION.to_string(),
            nodes,
            edges,
...
fn knowledge_object_to_graph_node_without_hash(
    ...
    // `GraphProjection::build` default path (no date pinning needed at that
...
mod tests {
    ...
    fn loose_list_child_paragraph_appears_under_list_not_at_page_level() {
        ...
        let artifact = GraphProjection.build(&workspace, &[]);
    ...
    fn build_workspace_with_contradiction(
        ...
        let artifact = GraphProjection.build_for_date(&workspace, &[], today);
    ...
    fn multiple_contradictions_tie_breaks_to_lex_smallest_id() {
        ...
        let workspace = WorkspaceAst { pages: vec![page] };
        let artifact = GraphProjection.build_for_date(&workspace, &[], None);
